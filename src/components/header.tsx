...
function Header({ defaultLogo = false, variant = "light" }: HeaderProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "mx-auto flex items-center justify-between relative h-full",
          getHeaderPadding(),
        )}
      >
        ...
        >
          ...
          <AnimatePresence>
            {isSocialMenuOpen && !isOpen && (
              <motion.div
                className={cn(
                  "absolute top-full right-0 mt-2 bg-white shadow-lg rounded-sm overflow-hidden z-50 border border-gray-100",
                  getSocialMenuWidth(),
                )}
                  ...
                  {socialLinks.map((link, index) => (
                    <motion.div
                      ...
                      style={{
                        // IE11 fallback for border color
                        borderBottomColor:
                          browserInfo.isIE && index !== socialLinks.length - 1
                            ? "#f3f4f6"
                            : undefined,
                      }}
                    >
                      <Link
                        href={link.href}
                        className={`flex items-center px-3 sm:px-4 py-3 sm:py-4 text-xs sm:text-sm tracking-wider text-gray-800 hover:bg-gray-50 hover:[font-style:italic] hover:text-black hover:scale-[1.02] rounded-sm w-full text-left min-h-[44px] ${getFocusRingClasses(true, variant)}`}
                        target="_blank"
              ...
              <div className="space-y-8 sm:space-y-10 md:space-y-12 lg:space-y-16 w-full max-w-4xl">
                {navLinks.map((link, index) => (
                  ...
                  >
                    ...
                    {/* Link text - Ensure text remains white */}
                    <Link
                      href={link.href}
                      className={cn(
                        ...
                        browserInfo.isIE
                          ? (e) => {
                              (e.currentTarget as HTMLElement).style.opacity =
                                "0.8";
                              (e.currentTarget as HTMLElement).style.fontStyle =
                                "italic";
                            }
                          : undefined
                      }
